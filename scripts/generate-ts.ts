...
function tsTypeFromSchema(
  ...
  if (schema.$ref) {
    ...
    if (refName) {
      return `${pascalCase(refName)}${required ? "" : " | undefined"}`;
    }
  }
  ...
  const type = (() => {
    ...
    if (t === "array" && normalized.items) {
      const itemType = tsTypeFromSchema(
        `${name}Item`,
        normalized.items,
        true,
        nested,
...
function buildInterface(
  ...
  const iface = `export interface ${name}${
    extendsList.length ? ` extends ${extendsList.join(", ")}` : ""
  } {\n${props.join("\n")}\n}\n`;
...
async function main() {
  ...
  if (names.has("PipelineStepBase")) {
    output += "export interface PipelineStep extends PipelineStepBase {}\n";
    if (names.has("CheckTask")) {
      output += "export interface PipelineStepCheck extends CheckTask {}\n";
    }
    if (names.has("BuildTask")) {
      output += "export interface PipelineStepBuild extends BuildTask {}\n";
    }
    output += "\n";
  }
