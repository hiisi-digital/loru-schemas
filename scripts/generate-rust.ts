...
import { loadSchemas, normalizeRef, resolveSchema, pascalCase, type JSONSchema } from "./schema_utils.ts";
...
function rustTypeFromSchema(
  ...
  const inner = (() => {
    ...
    if (t === "array" && normalized.items) {
      const itemType = rustTypeFromSchema(`${name}Item`, normalized.items, true, nested, defs).replace(/^Option<(.+)>$/, "$1");
      return `Vec<${itemType}>`;
    }
    if (t === "object") {
...
function buildStruct(name: string, schema: JSONSchema): { main: string; nested: string[] } {
  ...
  const props = Object.entries(normalized.properties ?? {}).map(([key, value]) => {
    ...
    const rustType = rustTypeFromSchema(`${name}${pascalCase(key)}`, value, fieldRequired, nested, defs);
...
async function main() {
  ...
  if (names.has("PipelineStepBase")) {
    ...
    for (const typ of impls) {
      ...
      output += `    fn name(&self) -> Option<&String> { self.name.as_ref() }\n`;
      output += `    fn targets(&self) -> Option<&Vec<String>> { self.targets.as_ref() }\n`;
      output += `    fn cmd(&self) -> Option<&String> { self.cmd.as_ref() }\n`;
      output += `    fn platform(&self) -> Option<&HashMap<String, Value>> { self.platform.as_ref() }\n`;
      output += `}\n\n`;
    }
  }
